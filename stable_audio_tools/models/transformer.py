...
class RMSNorm(nn.Module):
    ...
    def forward(self, x):
        # always bring gamma to dtype input
        weight = self.gamma.to(x.dtype)

        if not self.force_fp32:
            return F.rms_norm(x, x.shape[-1:], weight=weight, eps=self.eps)
        else:
            output = F.rms_norm(x.float(), x.shape[-1:], weight=weight.float(), eps=self.eps)
            return output.to(x.dtype)
...
class Attention(nn.Module):
    ...
    def apply_attn(self, q, k, v, causal = None, flex_attention_block_mask = None, flex_attention_score_mod = None, flash_attn_sliding_window = None, padding_mask = None, varlen_metadata = None):
        ...
        # FlashAttention GPU < Ampere
        if cc_major < 8:
            flash_attn_available = False
            flash_attn_varlen_available = False
            logging.info(f"[INFO] FlashAttention disabled automatically (CC {cc_major}.{cc_minor} < 8.0)")
